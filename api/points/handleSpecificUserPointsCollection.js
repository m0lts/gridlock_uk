...
export default async function handler(request, response) {
    ...
    try {
        ...
        try {
            const { specifiedUsers } = request.body; // Assuming specifiedUsers is an array of user emails

            const userTotalPoints = await dbCollection.aggregate([
                { $match: { userName: { $in: specifiedUsers } } }, // Filter points only for specified users
                { $unwind: "$points" },
                { $group: { _id: "$userName", totalPoints: { $sum: "$points.points" } } }
            ]).toArray();
